            ...
            OnSourceEvent = (@event, point) =>
            {
                ...
                switch (@event.Name)
                {
                    case "node-subscribe":
                        ...
                        if (_active)
                        {
                            var interval = @event.Data[1] << 8 + @event.Data[0];
                            _scheduler.AddSchedule(new CommandSchedule(
                                DateTime.Now.AddSeconds(interval),
                                "node-getstatus", 
                                NodeCommands.CMD_GET_NODE_STATUS, 
                                address,
                                @event.Data,
                                _bus, ScheduleTypes.Period,
                                interval
                                ));
                        }
                        else
                    ...
                    case "node-unsubscribe":
                        /*
                        if (_active)
                            _scheduler.Purge("node-getststatus");
                        else
                            */
                    ...
                    case "sensor-subscribe":
                        //if (Active)
                        //{
                        //    var interval = ((byte) @event.Data[1]) << 8 + (byte)@event.Data[0];
                        //    stack.PushName(@event.Source);
                        //    _scheduler.AddSchedule(new CommandSchedule(
                        //        DateTime.Now.AddSeconds(interval),
                        //        "sensor-read:" + @event.Source,
                        //        NodeCommands.CMD_READ_SENSOR,
                        //        address,
                        //        stack.Data,
                        //        _bus, ScheduleTypes.Period,
                        //        interval
                        ...
                        //else
                        {
                            //var interval = (ushort) (@event.Data[1] << 8 + @event.Data[0]);
                            stack.PushName(@event.Source);
                            stack.Push((byte ) 10); //Interval
                            stack.Push((ushort) 60);//Expires
                            _bus.SendCommand(NodeCommands.CMD_ADD_SENSOR_LISTENER, address, stack.Data);
                        }
                        break;
                    case "sensor-unsubscribe":
                        if (Active)
                            _scheduler.Purge("sensor-read:" + @event.Source);
                        else
                        {
                            stack.PushName(@event.Source);
                    ...
                    default:
                        ...
                        {
                            //Send device action
                            var devaction = new DeviceAction(@event.Source, @event.Name.Substring(7));
